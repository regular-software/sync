...
for (const [index, pkg] of manifests.entries()) {
  ...
  for (const section of ["dependencies", "optionalDependencies", "peerDependencies"]) for (const [name, spec] of Object.entries(pkg[section] ?? {})) if (spec.startsWith("file:") || spec.startsWith("link:") || spec === "catalog:") throw new Error(`${pkg.name}: non-publishable ${section} ${name}=${spec}`);
